...
export default function ProjectsPane({ slicer }: SlicerControlsProps) {
  ...
  return (
    <PanelSection title={`Projects (${slicer.localProjects.length})`} bodyPadding="8px">
      ...
      <div style={{ maxHeight: "230px", overflowY: "auto", display: "grid", alignContent: "start", gap: "6px", paddingRight: "2px" }}>
            ...
            >
              ...
              <details style={{ position: "relative" }}>
                <summary
                  ...
                  style={{ width: "30px", height: "36px", display: "grid", placeItems: "center", listStyle: "none", border: `1px solid ${theme.control.border}`, borderRadius: "5px", background: theme.control.lightBackground, fontSize: "20px", fontWeight: 800, lineHeight: 1, cursor: "pointer" }}
                >
                  ⋮
                </summary>
                <div style={{ position: "absolute", right: 0, top: "40px", zIndex: 5, minWidth: "110px", display: "grid", padding: "4px", border: `1px solid ${theme.control.buttonBorder}`, borderRadius: "6px", background: theme.control.lightBackground, boxShadow: "0 5px 14px hsl(42 20% 10% / 28%)" }}>
                  <button type="button" onClick={() => void slicer.handleOpenLocalProject(project.id)} style={{ padding: "6px 8px", border: 0, background: "transparent", textAlign: "left", cursor: "pointer" }}>Open</button>
                  <button type="button" onClick={() => { const name = window.prompt("Project name", project.name); if (name) void slicer.handleRenameLocalProject(project.id, name); }} style={{ padding: "6px 8px", border: 0, background: "transparent", textAlign: "left", cursor: "pointer" }}>Rename</button>
                  <button type="button" onClick={() => void slicer.handleExportLocalProject(project.id)} style={{ padding: "6px 8px", border: 0, background: "transparent", textAlign: "left", cursor: "pointer" }}>Export</button>
                  <button type="button" onClick={() => { if (window.confirm(`Delete “${project.name}” from this browser?`)) void slicer.handleDeleteLocalProject(project.id); }} style={{ padding: "6px 8px", border: 0, borderTop: `1px solid ${theme.control.border}`, background: "transparent", color: "hsl(5 70% 35%)", textAlign: "left", cursor: "pointer" }}>Delete</button>
                </div>
              </details>
            </article>
          );
